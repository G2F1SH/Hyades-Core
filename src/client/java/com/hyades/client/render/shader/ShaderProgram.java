...
import com.hyades.client.HyadesCoreClient;
...
/**
 * GLSL Shader 程序封装�? * <p>
 * 从资源文件加�?{@code .fsh} / {@code .vsh}，编译链接失败时优雅降级
...
public class ShaderProgram {
    ...
    /**
     * @param fragmentName 片段着色器文件名（不含扩展名，加载 {@code .fsh}�?     * @param vertexName   顶点着色器文件名（不含扩展名，加载 {@code .vsh}�?     */
    public ShaderProgram(String fragmentName, String vertexName) {
        ...
        if (GL20.glGetProgrami(this.programId, GL20.GL_LINK_STATUS) == GL11.GL_FALSE) {
            HyadesCoreClient.LOGGER.warn("Shader link failed: {}", GL20.glGetProgramInfoLog(this.programId));
            this.valid = false;
            this.cleanup(fragmentShader, vertexShader);
            return;
        }
    ...
    private static int compileShader(int type, String source) {
        if (source == null || source.isEmpty()) {
            HyadesCoreClient.LOGGER.warn("Skipping shader compilation: empty source (type={})", type);
            return 0;
        }
        ...
        if (GL20.glGetShaderi(shader, GL20.GL_COMPILE_STATUS) == GL11.GL_FALSE) {
            HyadesCoreClient.LOGGER.warn("Shader compile failed: {}", GL20.glGetShaderInfoLog(shader));
